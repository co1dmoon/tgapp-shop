    ...
    <div className="py-4">
      <div className="grid grid-cols-4 gap-3 px-4">
        {navButtons.map((button) => (
          <button
            key={button.id}
            className={`flex flex-col items-center text-[#ffff00] justify-center p-2 py-4 rounded-xl transition-colors ${currentSection === button.id
              ? 'bg-[#161616]'
              : ''
              }`}
          ...
          >
            ...
            <div className="font-primary font-thin text-[12px] text-white">{button.title}</div>
          </button>
        ))}
      </div>
    </div>
