...
const validationSchema = yup.object().shape({
  ...
  deliveryType: yup
    ...
    .required("Тип доставки обязателен"),
  payingType: yup
    .string()
    .oneOf(Object.values(PayingType), "Выберите корректный тип оплаты")
...
export default function OrderForm({
  ...
  setOrder: React.Dispatch<React.SetStateAction<"no" | "start" | "finish">>;
}) {
  const { user } = useTelegram();
  const { data: userData } = useUserByTelegramId({
    telegramId: user?.id.toString() ?? "805354266",
  });
    ...
    onSubmit: async (values) => {
      ...
      try {
        const deliveryAddress =
          values.deliveryType === DeliveryType.PICKUP
            ? selectedPickupPoint
              ? `${selectedPickupPoint.name}, ${selectedPickupPoint.address}`
              : "Самовывоз"
            : `${values.address}, под. ${values.entrance}, эт. ${values.floor}${
                values.intercom ? `, домофон: ${values.intercom}` : ""
              }, кв. ${values.apartment}`;
  ...
  const handleMoscowAddressChange = (value: string) => {
    formik.setFieldValue("address", "Москва, " + value);
  };
    ...
    return (
      <div className="p-4 h-400px">
        ...
        <div className="flex items-center justify-center h-64">
          <FaSpinner
            color="#ffff00"
            className="animate-spin text-[#0000ff] text-4xl"
          />
        </div>
      </div>
    );
      ...
      <form onSubmit={formik.handleSubmit} className="flex flex-col gap-4">
        <div className="flex flex-col gap-2">
          <label
            ...
            className="text-[10px] text-gray-400 font-primary"
          >
            Тип доставки*
          </label>
          <div className="relative">
            ...
            >
              ...
              <option className="text-[10px]" value={DeliveryType.MOSCOW}>
                Платная доставка курьером по Москве и Московской области
              </option>
              <option className="text-[10px]" value={DeliveryType.PICKUP}>
                Самовывоз
              </option>
            </select>
        ...
        <div className="flex flex-col gap-2">
          {(formik.values.deliveryType === DeliveryType.MOSCOW ||
            formik.values.deliveryType === DeliveryType.CDEK) && (
            <>
              {formik.values.deliveryType === DeliveryType.MOSCOW ? (
                <MoscowAddressInput
                  value={formik.values.address.replace("Москва, ", "")}
                  onChange={handleMoscowAddressChange}
            ...
            <div className="flex flex-col gap-2">
              ...
              {selectedPickupPoint && (
                <div className="bg-[#161616] p-4 rounded-2xl">
                  ...
                  <p className="font-primary text-[12px] text-gray-400">
        ...
        <div className="rounded-xl bg-[#161616] p-2">
          ...
          {cart.items.map((item) => (
            <div
              key={item.productId}
              className="flex gap-2 items-center justify-between py-2 border-b border-gray-700"
            >
              <img
                src={
                  (item.image as string) ?? "/images/categories/игровые-пк.png"
                }
                alt={item.name}
                className="w-[40px] h-[40px] rounded-xl"
              />
              <div className="h-full w-full flex items-start gap-2 text-[10px] font-primary">
                {!isPC(item.category?.name ?? "") ? (
                  <div className="rounded-xl bg-[#222222] px-2 py-1">
                    {item.name}
                  </div>
                ) : (
                  getSpecs(item.specs ?? "{}").map((spec, index) => (
                    <div
                      key={index}
                      className="rounded-xl bg-[#222222] px-2 py-1"
                    >
                      {spec}
                    </div>
                  ))
          ...
          <div className="flex items-center justify-between mt-2 ">
            ...
            <p className="font-primary text-[16px]">
        ...
        <button
          type="submit"
          className="bg-[#ffff00] text-black font-display rounded-2xl p-2"
        >
          Оформить заказ
        </button>
      </form>
